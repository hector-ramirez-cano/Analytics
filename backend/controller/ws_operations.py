...
async def syslog_rt(data_out_queue: janus.Queue, queue: asyncio.Queue, finished: threading.Event):
    ...
    Args:
        ...
        queue (asyncio.Queue): Input queue from the Syslog backend from which new events will be recieved
...
async def alerts_rt(data_out_queue: janus.Queue, queue: asyncio.Queue, finished: threading.Event):
    ...
    Args:
        data_out_queue (janus.Queue): Queue to which new messages will be posted to
        queue (asyncio.Queue): Input queue from the Alerts backend from which new events will be recieved
        finished (threading.Event): Event indicating the end of the program, for graceful shutdown.
    """
    while not finished.is_set():
        try:
