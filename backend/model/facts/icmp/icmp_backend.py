...
LOSS_PATTERN = re.compile(r"(\d+(?:\.\d+)?)% packet loss")
...
def __ping_host_once(host: str):
    ...
    if result.returncode == 0:
        rtt      = __parse_float(re.findall(RTT_PATTERN, result.stdout)[0][1], 0.0)
        loss     = __parse_float(re.findall(LOSS_PATTERN, result.stdout)[0], 0.0)
...
def __parse_float(value: str, default: float):
    try:
        return float(value)
